  ...
  if (user) {
    try {
      ...
      if (dbUser) {
        ...
        // Fetch role specifically for the organization that owns this team
        const { data: member } = await adminClient.from('organization_members').select('role').eq('user_id', dbUser.id).eq('org_id', team.org_id).single()
